...
public class LogsToJSONConverterTest {
    ...
    public void testFormatJSONRecord() throws Exception {
        TelemetryProducer.LogsToJSONConverter df = new TelemetryProducer.LogsToJSONConverter("time a1 b1 a2 b2", 42, Arrays.asList("a", "b"));
    ...
    @Test
    public void testLimitData() throws Exception {
        TelemetryProducer.LogsToJSONConverter df = new TelemetryProducer.LogsToJSONConverter("time a1 b1 a2 b2", 42, Arrays.asList("a"));
        Assert.assertEquals(2, df.getCarsCount());
        ...
        Assert.assertEquals(1, car1.getInt("id"));
        Assert.assertEquals(1, car1.getJSONObject("sensors").getInt("a"));
        Assert.assertTrue(!car1.getJSONObject("sensors").has("b"));

        JSONObject car2 = jsonObject.getJSONArray("cars").getJSONObject(1);
        Assert.assertEquals(2, car2.getInt("id"));
        Assert.assertEquals(3, car2.getJSONObject("sensors").getInt("a"));
        Assert.assertTrue(!car2.getJSONObject("sensors").has("b"));
    }
}
